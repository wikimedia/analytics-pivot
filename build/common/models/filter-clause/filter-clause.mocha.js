...
describe('FilterClause', function () {
    it('is an immutable class', function () {
        immutable_class_tester_1.testImmutableClass(filter_clause_1.FilterClause, [
            {
    ...
    describe("isLessThanFullDay", function () {
        it("works with less than full day", function () {
            var clause = filter_clause_1.FilterClause.fromJS({
                expression: { op: 'ref', name: 'time' },
                selection: {
                    op: 'literal',
            ...
            chai_1.expect(clause.isLessThanFullDay()).to.equal(true);
        });
        it("returns false for exactly one day", function () {
            var clause = filter_clause_1.FilterClause.fromJS({
                expression: { op: 'ref', name: 'time' },
                selection: {
                    op: 'literal',
            ...
            chai_1.expect(clause.isLessThanFullDay()).to.equal(false);
        });
    });
});
